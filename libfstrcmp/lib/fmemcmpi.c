...
/*
 * Because fstrcmpi is typically called multiple times, attempt to
...
int
fmemcmpi(const void *data1, size_t size1, const void *data2, size_t size2)
{
    struct context  ctxt;
    int             i;
    size_t          fdiag_len;
     ...
     *
     * Even though we return int, we have to do the calculation as long
     * (or, in this case, size_t) to avoid numerical overflow on 16-bit
     * machines.
     */
