...
static void ssv_attach_ssv6006_mac(struct ssv_hal_ops *hal_ops)
{
...
void ssv_attach_ssv6006(struct ssv_softc *sc, struct ssv_hal_ops *hal_ops)
{
    ...
    *((u32 *)&fpga_tag[0])= __be32_to_cpu(regval);
    ...
    if (strstr(sc->platform_dev->id_entry->name, SSV6006MP)){
        ...
        printk(KERN_INFO"Load SSV6006C HAL MAC function \n");
    } else {
    ...
    if (chip_type == CHIP_TYPE_CHIP){
        ...
        if (strstr(sc->platform_dev->id_entry->name, SSV6006MP)){
            printk(KERN_INFO"Load SSV6006C HAL BB-RF function \n");
