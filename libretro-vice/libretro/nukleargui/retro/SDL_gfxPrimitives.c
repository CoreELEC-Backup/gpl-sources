...
RSDL_gfxPrimitives.c: graphics primitives for SDL surfaces
...
/*
#include "RSDL_rotozoom.h"
#include "RSDL_gfxPrimitives_font.h"
#include "RSDL_gfxBlitFunc.h"
*/
...
\returns Returns 0 on success, -1 on failure.
*/
int _HLineTextured(RSDL_Surface * dst, Sint16 x1, Sint16 x2, Sint16 y, RSDL_Surface *texture, int texture_dx, int texture_dy)
{
	Sint16 left, right, top, bottom;
	Sint16 w;
	Sint16 xtmp;
	...
	int texture_y_start;    
	RSDL_Rect source_rect,dst_rect;
	...
	*/
	...
	if (w <= texture->w -texture_x_walker){
		...
		source_rect.x = texture_x_walker;
		dst_rect.x= x1;
		result = (RSDL_BlitSurface  (texture, &source_rect , dst, &dst_rect) == 0);
...
This operation use internally RSDL_BlitSurface for lines of the source texture. It supports
alpha drawing.

To get the best performance of this operation you need to make sure the texture and the dst surface have the same format
(see  http://docs.mandragor.org/files/Common_libs_documentation/SDL/RSDL_Documentation_project_en/sdlblitsurface.html).
...
\returns Returns 0 on success, -1 on failure.
*/
int texturedPolygonMT(RSDL_Surface * dst, const Sint16 * vx, const Sint16 * vy, int n, 
	RSDL_Surface * texture, int texture_dx, int texture_dy, int **polyInts, int *polyAllocated)
{
	int result;
	int i;
	int y, xa, xb;
...
\returns Returns 0 on success, -1 on failure.
*/
int texturedPolygon(RSDL_Surface * dst, const Sint16 * vx, const Sint16 * vy, int n, RSDL_Surface *texture, int texture_dx, int texture_dy)
{
	/*
	* Draw
	*/
...
*/
static RSDL_Surface *gfxPrimitivesFont[256];
...
void gfxPrimitivesSetFont(const void *fontdata, Uint32 cw, Uint32 ch)
{
	...
	for (i = 0; i < 256; i++) {
		if (gfxPrimitivesFont[i]) {
			RSDL_FreeSurface(gfxPrimitivesFont[i]);
...
\returns Returns 0 on success, -1 on failure.
*/
int characterColor(RSDL_Surface * dst, Sint16 x, Sint16 y, char c, Uint32 color)
{
	Sint16 left, right, top, bottom;
	Sint16 x1, y1, x2, y2;
	RSDL_Rect srect;
	...
	*/
	if (gfxPrimitivesFont[ci] == NULL) {
		gfxPrimitivesFont[ci] =
			RSDL_CreateRGBSurface(RSDL_SWSURFACE | RSDL_HWSURFACE | RSDL_SRCALPHA,
	...
	*/
	if ((gfxPrimitivesFontColor[ci] != color) || (forced_redraw)) {
		...
		* Redraw character 
		*/
		RSDL_SetAlpha(gfxPrimitivesFont[ci], RSDL_SRCALPHA, 255);
		gfxPrimitivesFontColor[ci] = color;

		/* Lock font-surface */
		if (RSDL_LockSurface(gfxPrimitivesFont[ci]) != 0)
			return (-1);
	...
	*/
	result = RSDL_BlitSurface(gfxPrimitivesFont[ci], &srect, dst, &drect);
...
\returns Returns 0 on success, -1 on failure.
*/
int characterRGBA(RSDL_Surface * dst, Sint16 x, Sint16 y, char c, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	/*
	* Draw 
	*/
...
\returns Returns 0 on success, -1 on failure.
*/
int stringColor(RSDL_Surface * dst, Sint16 x, Sint16 y, const char *s, Uint32 color)
{
	int result = 0;
	Sint16 curx = x;
	Sint16 cury = y;
...
\returns Returns 0 on success, -1 on failure.
*/
int stringRGBA(RSDL_Surface * dst, Sint16 x, Sint16 y, const char *s, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	/*
	* Draw 
	*/
...
\returns Returns 0 on success, -1 on failure.
*/
int bezierColor(RSDL_Surface * dst, const Sint16 * vx, const Sint16 * vy, int n, int s, Uint32 color)
{
	int result;
	int i;
...
\returns Returns 0 on success, -1 on failure.
*/
int bezierRGBA(RSDL_Surface * dst, const Sint16 * vx, const Sint16 * vy, int n, int s, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	/*
	* Draw 
	*/
