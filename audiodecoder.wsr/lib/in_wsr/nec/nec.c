...
OP( 0x9d, i_popf      ) { UINT32 tmp; POP(tmp); ExpandFlags(tmp); CLK(3); nec_int(); }
OP( 0x9e, i_sahf      ) { UINT32 tmp = (CompressFlags() & 0xff00) | (I.regs.b[AH] & 0xd5); ExpandFlags(tmp); CLK(4); nec_int(); }
...
OP( 0xfb, i_ei    ) { SetIF(1);			CLK(4);	nec_int(); }
...
//		nec_ICount++;
	...
	return nec_Cycles - nec_ICount;
}

int nec_getcycles(void)
{
	return nec_Cycles - nec_ICount;
}
...
void nec_yield(void)
{
	nec_Cycles = nec_Cycles - nec_ICount;
	nec_ICount = 0;
}
